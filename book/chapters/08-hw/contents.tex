...
\abbrev{EDVAC}{Electronic Discrete Variable Automatic Computer} (\textit{Electronic Discrete Variable Automatic Computer}) în 1945 ce
...
\begin{itemize}
  \item x86 - disponibil atât pe 32 de biți (referit ca x86), cât și pe 64
          ...
          CISC. \labelindexref{Figura}{fig:hw:x86}\footnote{https://commons.wikimedia.org/wiki/File:Modified-pc-case.png (CC BY-SA)} prezintă un calculator cu procesor cu arhitectură x86.
  ...
  \item ARM - disponibil atât pe 32 de biți (referit ca ARMv7), câț și pe
          ...
          \labelindexref{Figura}{fig:hw:arm}\footnote{\url{https://en.wikipedia.org/wiki/Nexus_5\#/media/File:Nexus_5_Front_View.png} (CC BY 2.5)} prezintă un smartphone cu procesor cu arhitectura
...
\begin{itemize}
  ...
  \item ARM are o arhitectură integrată denumită și SoC (\textit{System on Chip}):
          ...
          schimbate separat. Vom detalia aceste sisteme în \labelindexref{Capitolul}{ch:embed}.
\end{itemize}
...
\labelindexref{Figura}{fig:hw:server}\footnote{\url{https://commons.wikimedia.org/wiki/File:Supermicro_SBI-7228R-T2X_blade_server.jpg} (CC BY-SA 4.0)} este ilustrat un server de dimensiune 1U
...
pentru a adresa portabilitatea fizică (vezi \labelindexref{Figura}{fig:hw:laptop}\footnote{\url{https://commons.wikimedia.org/wiki/File:Lenovo_G500s_laptop-2905.jpg} (CC BY-SA 4.0)}). De
...
\textit{Sistemele embedded} (detaliate în \labelindexref{Capitolul}{ch:embed}), de obicei,
...
\labelindexref{Figura}{fig:hw:embed}\footnote{\url{https://commons.wikimedia.org/wiki/File:Embedded_World_2016,_Nordic_RF51822.jpg} (CC0)} prezintă un astfel de sistem embedded
(încorporat).
...
Componentele sunt prezentate vizual în \labelindexref{Figura}{fig:hw:components}\footnote{\url{https://commons.wikimedia.org/w/index.php?curid=4023664} (CC BY 2.5)}. Le enumerăm în continuare:
...
\begin{enumerate}
  ...
  \item CPU (\labelindexref{Figura}{fig:hw:cpu}\footnote{\url{https://commons.wikimedia.org/wiki/File:Intel_CPU_Core_i7_6700K_Skylake_top.jpg} (CC BY-SA)}) - procesorul (unitatea de procesare) execută instrucțiunile
  ...
  \item memorie (\labelindexref{Figura}{fig:hw:ram}\footnote{\url{https://commons.wikimedia.org/wiki/File:DDR_RAM-3.jpg} (Public domain)}) - unitatea de memorie care este de tip RAM
...
\end{figure}
  ...
  \item sursă de alimentare a unității de calcul - transformă curentul
        ...
        \item unitate optică (CD/DVD - ROM) \abbrev{ROM}{Read-Only Memory} (\labelindexref{Figura}{fig:hw:dvd}\footnote{\url{https://commons.wikimedia.org/wiki/File:ASUS_CD-ROM_CD-S520-A4_20080821.jpg} (CC BY 3.0)}) -
  ...
  \item unitate de stocare (HDD \abbrev{HDD}{Hard Disk Drive} - \textit{Hard Disk
    Drive}) (\labelindexref{Figura}{fig:hw:hdd}\footnote{\url{https://commons.wikimedia.org/wiki/File:Hard_Drive_(11644419853).jpg} (CC BY 2.0)}) - este un dispozitiv de intrare/ieșire pentru
...
\item placă de bază (\labelindexref{Figura}{fig:hw:mb}\footnote{\url{https://commons.wikimedia.org/wiki/File:Mother_board.JPG} (CC BY-SA 4.0)}) - reprezintă elementul central de conexiune între
  ...
  \item monitor - conectat la placa de extensie (numită și placă grafică
    sau GPU \abbrev{GPU}{Graphics Processing Unit} în acest caz - \textit{Graphics Processing Unit})
...
Sistemul de operare (\textit{operating system} - OS\abbrev{OS}{Operating System}) este primul program
...
\begin{itemize}
  \item \textit{autodiscovery} (descoperire automată de către sistemul de operare)
                  ...
                  \labelindexref{Capitolul}{ch:boot}) care testează toate
...
introdus și ce opțiuni de configurare oferă, sunt prezentate în \labelindexref{Capitolul}{ch:boot}.
...
USB (\textit{Universal Serial Bus}). În \labelindexref{Figura}{fig:hw:usb} se poate observa forma unui port USB. Acesta vine în mai
...
În \labelindexref{Figura}{fig:hw:audio}\footnote{\url{https://commons.wikimedia.org/wiki/File:A_VIA_Envy_Sound_Card_5.1_6_Channels_(VIA_VT1617A).jpg} (CC BY-SA 4.0)} este reprezentată o placă de sunet 5.1
...
\begin{itemize}
  ...
  \item interfața disponibilă: în general cupru RJ-45 (protocol ethernet).
          ...
          Pluggable Transceiver} (SFP+) (\textit{Small Form-factor Pluggable Transceiver}). Atenție pentru a utiliza suportul
...
aceasta trebuie activată din BIOS (vezi \labelindexref{Capitolul}{ch:boot}).
...
BIOS, ca și virtualizarea (vezi \labelindexref{Capitolul}{ch:boot}).
