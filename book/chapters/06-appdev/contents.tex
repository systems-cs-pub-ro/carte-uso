...
magnetice sau cartele perforate, ca în \labelindexref{Figura}{fig:appdev:card}\footnote{\url{https://en.wikipedia.org/wiki/Computer_programming_in_the_punched_card_era\#/media/File:FortranCardPROJ039.agr.jpg} (CC BY-SA 2.5)}. Odată cu evoluția procesoarelor și a mediilor de
...
PHP \abbrev{PHP}{PHP Hypertext Preprocessor} (\textit{PHP Hypertext Preprocessor}) este un limbaj folosit preponderent pentru dezvoltarea de servere web. Mai
...
JIT\abbrev{JIT}{just-in-time} - \textit{just-in-time}).
...
La o primă vedere, acestea sunt similare cu editoarele de text. IDE-urile \abbrev{IDE}{Integrated Development Environment} (\textit{Integrated Development Environment}) au, în schimb, anumite
...
aplicație Python care face cereri HTTP\abbrev{HTTP}{Hypertext Transfer
...
La folosirea \cmd{gcc}, sistemul analizează fișierele de intrare și hotărăște dacă
limbajul compilat este C sau C++. Pentru a eficientiza procesul, se poate folosi
direct \cmd{g++}, adică compilatorul de C++ integrat în GCC, ca în \labelindexref{Listing}{lst:appdev:simple-cpp}.
...
arhive (jar \abbrev{JAR}{Java Archive} - \textit{Java Archive}) care sunt incluse în aplicațiile pe care le
...
Unul dintre cele mai cunoscute utilitare pentru depanare este GDB \abbrev{GDB}{GNU Debbuger} (\textit{GNU Debugger}). GDB oferă suport pentru
...
GNU GPL \abbrev{GNU GPL}{GNU General Public
...
MIT \abbrev{MIT}{Massachusetts Institute of Technology} (\textit{Massachusetts Institute of Technology}) este o licență mai
