...
\begin{figure}[htbp]
  ...
  % TODO - schemă cu folosirea unui limbaj intermediar
\end{figure}
...
Similar cu Vim, pentru instalarea Nano, Pico sau Emacs, este suficient să rulăm în
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{chapters/06-appdev/img/screenshot.png}
...
elementele folosite și dependența între fișiere, ca în \labelindexref{Listing}{lst:appdev:multiple-compile}
...
Dependențele sunt fișiere și/sau reguli necesare pentru a putea rula o regulă.
...
nume și se va rula acea regulă (dacă dependențele ei sunt îndeplinite, dacă nu,
...
\cmd{pip} este un utilitar care permite manipularea pachetelor Python, cum ar fi
instalarea sau dezinstalarea unor biblioteci. \labelindexref{Listing}{lst:appdev:pip} conține un exemplu de folosire utilitarului \cmd{pip}.

\begin{screen}[caption={Instalarea de pachete Python folosind pip},label={lst:appdev:pip}]
...
pe site. Platforma se poate instala și instalând pachetul \texttt{nodejs}, dar se va descărca o versiune foarte veche a platformei. De aceea
recomandăm descărcarea binarului. \labelindexref{Listing}{lst:appdev:install-nodejs} prezintă instalarea NodeJS versiune 8 pentru Linux pe 64 de
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{chapters/06-appdev/img/screenshot.png}
  \caption{Interfață de depanare într-un IDE}
  \label{fig:appdev:ide-debug}
  % TODO - poză cu debug
\end{figure}

Unul din cele mai cunoscute utilitare pentru depanare este GDB. GDB oferă suport pentru
