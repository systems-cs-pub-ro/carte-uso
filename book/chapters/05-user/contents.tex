...
(UAC\abbrev{UAC}{User Acount Control}). \textit{User Account Control} este o tehnologie
...
comanda \cmd{exit}, așa cum este ilustrat mai jos:
...
parametru. La rularea comenzii \cmd{su} se cere parola noului utilizator; fără
introducerea parolei acelui utilizator nu se poate schimba utilizatorul.

* listare de comenzi cu su

O formă uzuală de rulare a comenzii \cmd{su} este cu opțiunea \texttt{-l} sau \texttt{-} care schimbă mediul de
...
intermediul opțiunii \texttt{-c}. În felul acesta se rulează direct comanda primită ca
...
mod obișnuit utilizatorul \texttt{root}. În listarea de mai jos avem un exemplu de
...
executăm ca \texttt{root}, ca în exemplele de mai jos:
...
\file{/etc/sudoers}. De exemplu, utilizatorul student poate rula comenzi ca \texttt{root}
...
utilizatorilor din grupul admin sau grupul \cmd{sudo} să poată rula comenzi ca \texttt{root}, ca mai jos:
...
\begin{note}[Folosire tee pentru a scrie într-un fișier privilegiat]
...
parole este reprezentată în Linux de fișierul /etc/shadow. Fișierul \file{/etc/shadow}
...
folosind utilitarul passwd trebuie să introducă parola la promptul afișat. Mai
jos avem exemple de folosire a comenzii \cmd{passwd} pentru gestiunea conturilor:
...
autentificare, putem folosi comanda \cmd{usermod} ca mai jos:
...
folosi utilitarul \cmd{chpasswd}, ca în exemplu de mai jos:
...
Astfel, în listarea de mai jos avem următoarele:
...
de comenzi \cmd{chown} și \cmd{chgrp}:
...
Bitul \texttt{setuid} este folosit atunci când anumite utilitare necesită privilegii
...
În exemplul de mai jos arătăm cum activarea bitului \texttt{setgid} pe un director duce
...
adăugăm șirul \texttt{init=/bin/bash}, ca mai jos:
...
cu permisiuni de \texttt{root}. În acest shell vom rula comenzile de mai jos pentru
