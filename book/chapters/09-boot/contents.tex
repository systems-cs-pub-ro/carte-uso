...
% Reviewers: Ștefan Bucur, Lucian Cojocar, Octavian Purdilă, Cosmin Rațiu, Mihai-Drosi Câju

\chapter{Pornirea sistemului}
\label{ch:boot}

Atunci când un sistem hardware pornește, acesta urmărește încărcarea sistemului de operare, pornirea acestuia și a proceselor de bază.
Această acțiune se cheamă \textit{booting} sau \textit{bootstrapping}: modul în care un sistem, din momentul în care este alimentat și pornit fizic, ajunge să ofere o interfață de lucru utilizatorului.
Procesul de boot/booting se referă la pașii urmați de la pornirea electrică (\textit{power on}) până la pornirea proceselor.

Înțelegerea procesului de boot este utilă pentru un utilizator dacă dorește să își configureze modul în care este configurat hardware-ul sistemului, ce sistem de operare rulează, opțiuni de rulare a sistemului de operare și ce procese sunt create la pornire (\textit{startup}).
Pentru un utilizator avizat, a ști informații despre anatomia procesului de boot și a secvența de pași urmați este important pentru a depana probleme ce pot apărea sau pentru a îmbunătăți aspecte precum timpul de pornire, securitatea sistemului, consumul de resurse.

După pornirea fizică (power on) a sistemului, este încărcat firmware-ul de boot (numit BIOS pe arhitectura x86), apoi este identificat un dispozitiv bootabil, este încărcat bootloaderul, apoi sistemul de operare și apoi sunt pornite procesele de început (\textit{startup processes}).
În \labelindexref{Figura}{fig:boot:sequence} este prezentat un sumar al pașilor urmați în secvența de boot, urmând să îi detaliem în secțiunile de mai jos:
...
În cadrul secvenței de boot, fiecare pas, descris în \labelindexref{Figura}{fig:boot:sequence}, are rolul său.
...
Firmware-ul de boot este numit BIOS (sau UEFI \abbrev{UEFI}{Unified Extensible Firmware Interface} - \textit{Unified Extensible Firmware Interface}) pe arhitectura x86.
...
O dată încărcat sistemul de operare, acesta încarcă și driverele de dispozitiv (\textit{device drivers}).
...
\begin{itemize}
  ...
  \item permite configurarea componentelor și opțiunilor hardware ale sistemului
  \item blochează acele configurații astfel încât să nu poată fi schimbate de sistemul de operare
  \item generează configurația pentru descoperirea și inițializarea hardware-lui de către sistemul de operare
  \item descoperă dispozitivele ce furnizează un bootloader
  \item lansează, posibil automat, bootloaderul
\end{itemize}

Firmware-ul de boot este rulat în momentul pornirii sistemului (adică la apăsarea pe butonul de pornire).
Partea de verificare a componentelor hardware poartă numele de POST \abbrev{POST}{Power-On Self Test} (\textit{Power-On Self Test}) pe arhitectura x86;
dacă sunt defecte hardware acestea vor fi semnalizate de firmware-ul de boot, în general prin mesaje sonore sau vizuale simple.

Firmware-ul de boot este parte a hardware-ului sistemului și vine ,,preinstalat''.
 În mod obișnuit, firmware-ul de boot este furnizat de producătorul plăcii de bază care conține și chipul flash.

Firmware-ul de boot permite realizarea de configurări la nivelul sistemului și hardware-ului.
Mai jos sunt o serie de configurări permise de firmware-ul de boot:
...
Pe arhitectura x86, firmware-ul de boot poartă numele de BIOS (\textit{Basic Input-Output System}).
...
\labelindexref{Figura}{fig:boot:bios-uefi} conține screenshoturi cu un ecran de boot BIOS\footnote{\url{https://commons.wikimedia.org/wiki/File:Bios-configuracion-smart-habilitada.png} (CC BY-SA 3.0)} și unul de boot UEFI\footnote{\url{https://commons.wikimedia.org/wiki/File:Bios\_asus.jpg} (CC BY-SA 4.0)}.
...
Aceste configurații sunt salvate într-un chip de memorie dedicat numit CMOS \abbrev{CMOS}{Complementary Metal-Oxide-Semiconductor} (de la \textit{Complementary Metal-Oxide-Semiconductor}, tehnologia folosită pentru tranzistoarele ce compun circuitele integrate moderne).
Această memorie este nevolatilă și își păstrează informațiile după resetarea sistemului.
Chipul de memorie este alimentat de o baterie dedicată numită baterie CMOS, similară cu cea din \labelindexref{Figura}{fig:boot:cmos-battery}\footnote{\url{https://commons.wikimedia.org/wiki/File:Yakumo\_Notebook\_536S\_-\_CR2032\_backup\_battery\_on\_motherboard-4667.jpg} (\textcopyright{} Raimond Spekking / CC BY-SA 4.0 (via Wikimedia Commons))}.
...
Aceasta este o formă facilă de resetarea a parolei de BIOS, în cazul în care o astfel de parolă este configurată, așa cum vom preciza în \labelindexref{Secțiunea}{sec:boot:pass-reset}.

Pe lângă interfața grafică mai prietenoasă, UEFI folosește o nouă schemă de partiționare a discurilor numită GPT \abbrev{GPT}{GUID Partition Table} (\textit{GUID Partition Table}) spre deosebire de schema MBR \abbrev{MBR}{Master Boot Record} (\textit{Master Boot Record}) utilizată de BIOS.
...
Vom reveni la schemele de partiționare GUID și MBR în \labelindexref{Secțiunea}{sec:boot:bootdev:scheme};
detalii vom prezenta și în \labelindexref{Secțiunea}{sec:storage:partition}.
...
UEFI folosește o partiție dedicată numită ESP \abbrev{ESP}{EFI System Partition} (\textit{EFI System Partition}), formatată cu sistem de fișiere FAT, în care stochează imagini de firmware (numite imagini EFI) care rețin codul bootloaderului ce va fi încărcat la următorul pas.
...
Datorită flexibilității, sistemele PC \abbrev{PC}{Personal Computer} (\textit{Personal Computer}) moderne folosesc din ce în ce mai mult UEFI, cu probabilitate mică de a mai întâlni, în următorii ani, sisteme care folosesc BIOS.
...
Pe sistemele PC o alternativă de firmware de boot este coreboot (și alternativa libreboot).
...
\labelindexref{Figura}{fig:boot:bios-boot-order}\footnote{\url{https://commons.wikimedia.org/wiki/File:Bios-configuracion-orden-arranque.png} (CC BY-SA 3.0)} afișează ecranul BIOS pentru stabilirea ordinii dispozitivelor bootabile.
...
Atunci când firmware-ul va trece la pasul următor va trece, în ordine, prin dispozitivele de boot configurate și va boota de pe primul disponibil.
 Dispozitive de boot pot fi discuri, USB flash drive-uri, CD ROM-drive sau poate fi vorba de bootare din rețea folosind PXE \abbrev{PXE}{Preboot eXecution Environment} (\textit{Preboot eXecution Environment}).
...
Această secvență este, de obicei, bootloaderul, despre care vom discuta detaliat în \labelindexref{Secțiunea}{sec:boot:bootloader}.

PXE este o metodă prin care putem configura pornirea unui sistem de operare de pe un alt sistem (server) accesibil prin rețea.
După o configurare specifică, pe care nu o vom detalia aici, o stație poate încărca sistemul de operare de pe alt sistem.

Despre dispozitive bootabile vom discuta mai multe în \labelindexref{Secțiunea}{sec:boot:bootdev}.
...
Mai multe despre certificate digitale vom vorbi în \labelindexref{Capitolul}{ch:sec}.
...
Bootloaderul se găsește pe un dispozitiv de boot, un suport de stocare pentru imaginea de bootloader.
...
GPT (\textit{GUID Partition Table}) este o schemă modernă de partiționare pe PC.
...
Dacă analizăm primul sector dintr-un disc, putem observa informații despre bootloader și prezența octeților \texttt{0x55AA} ca în \labelindexref{Listing}{lst:boot:magic}.
...
Spațiul disponibil în primul sector este limitat, doar \texttt{512} octeți.
De aceea, această primă parte de bootloader poate face operații minime.
Astfel că rolul său este să încarce partea principală din bootloader, numită și \textit{2nd stage bootloader}, care realizează apoi încărcarea sistemului de operare, așa cum vom preciza în \labelindexref{Secțiunea}{sec:boot:bootloader}.

\subsection{Încărcarea bootloaderului pentru UEFI}
\label{sec:boot:bootdev:uefi}

UEFI are un mod compatibil BIOS numit și CSM \abbrev{CSM}{Compatibility Support Module} (\textit{Compatibility Support Module}) care duce la bootarea într-un mod similar BIOS: încărcare \textit{1st stage bootloader} din MBR, apoi încărcare \textit{2nd stage bootloader}.
...
De exemplu, putem avea fișierul \file{/efi/boot/bootx64.efi} pentru booting pe un sistem PC.
...
De exemplu, vom adăuga \file{/efi/fedora/grubx64.efi} pentru porni bootloaderul GRUB care va încărca imaginea de sistem de operare pentru o instalare Fedora Linux.

În Linux, pentru gestiunea intrărilor din partiția EFI se poate folosi utilitarul \cmd{efibootmanager}.
...
În general, discurile și partițiile sunt, în Linux, intrări în directorul \file{/dev}.
 De exemplu \file{/dev/sda} este primul disc al sistemului, \file{/dev/sdb} al doilea disc, \file{/dev/sdc} al treilea disc.
Apoi partițiile sunt numerotări ale acestor discuri: \file{/dev/sda1} este prima partiție a primului disc, \file{/dev/sdb3} este a treia partiție a celui de-al doilea disc.

Atunci când folosim GRUB ca bootloader, numele discurilor au alt format, pe echivalentul celor de mai jos:

\begin{itemize}
  \item \texttt{(hd0)} este primul disc, echivalentul \file{/dev/sda}
  \item \texttt{(hd1)} este al doilea disc, echivalentul \file{/dev/sdb}
  \item \texttt{(hd0,1)} sau \texttt{hd0,msdos1} este prima partiție a primului disc, echivalentul \file{/dev/sda1}
  \item \texttt{(hd1,3)} sau \texttt{hd1,msdo3} este a treia partiție a celui de-al doilea disc, echivalentul \file{/dev/sdb3}
\end{itemize}
...
Întrucât ordinea discurilor poate diferi prin mutarea discurilor sau prin adăugarea unui disc nou, cele două moduri de denumire de mai sus sunt variabile.
 Pentru a menține o denumire fixă, se folosește un identificator unic al partiției un șir de caractere numit UUID \abbrev{UUID}{Universally Unique Identifier} (Universally Unique Identifier).
Pe Linux putem obține acest identificator cu ajutorul comenzii \cmd{blkid} sau listând conținutul directorului \file{/dev/disk/by-uuid}, ca în \labelindexref{Listing}{lst:boot:dev-name}.
...
În \labelindexref{Listing}{lst:boot:dev-name} de mai sus, se observă că sistemul de fișiere aflat pe partiția \file{/dev/sda1} are ca identificator \texttt{a14d0991-a3d8-48d6-ac8c-327d1a524501}.
Acest identificator este folosit și în fișierul \file{/etc/fstab}, fișier unde se configurează sistemele de fișiere ce vor fi montate, așa cum vom detalia în \labelindexref{Secțiunea}{sec:storage:fs}.
...
Concret, dacă un sistem are instalat Linux și Windows, sistemul va avea configurat bootloaderul specific Linux (GRUB) ca 1st stage și 2nd stage bootloader.
...
Când folosim UEFI, GRUB va plasa un fișier imagine \texttt{.efi} în \file{/efi/boot/bootx64.efi}.
...
În \labelindexref{Figura}{fig:boot:boot-grub} avem doar opțiuni de boot pentru Linux (Ubuntu).
...
În acest caz utilizatorul va apăsa tasta \texttt{e} și va accesa o nouă interfață ca în \labelindexref{Figura}{fig:boot:edit-grub}.
...
O utilizare a editării opțiunilor de boot este recuperarea parolei contului \texttt{root} folosind opțiunea \texttt{init=/bin/bash} așa cum am precizat în \labelindexref{Secțiunea}{sec:user:linux-password-recovery}.
Detalii despre opțiunile de boot corespunzătoare nucleului sistemului de operare prezentăm în \labelindexref{Secțiunea}{sec:boot:os}.
...
Opțiunile afișate de GRUB sunt încărcate din fișierul \file{/boot/grub/grub.cfg}, fișierul de configurare GRUB.
Acest fișier nu se recomandă să fie editat manual;
 ...
 Comanda \cmd{update-grub} consultă partițiile sistemului, detectează sisteme de operare și imagini de kernel, consultă configurările din sistem și apoi generează fișierul \file{/boot/grub/grub.cfg}.

Configurările GRUB, care devin persistente după rularea comenzii \cmd{update-grub}, se realizează în fișierul \file{/etc/default/grub} și în fișierele din \file{/etc/grub.d/}.
În fișierul \file{/etc/default/grub} sunt variabile GRUB care afectează comportamentul acestuia.
\labelindexref{Listing}{lst:boot:grub-cfg} din fișierul \file{/etc/default/grub}.
...
Observăm la începutul fișierului comentariul care spune că aceste configurări sunt apoi marcate persistente în fișierul de configurare \file{/boot/grub/grub.cfg} după rularea comenzii \cmd{update-grub}.
Opțiunile de mai sus reprezintă:

\begin{itemize}
  \item \texttt{GRUB\_DEFAULT}: indexul opțiunii implicite de boot a GRUB;
    în ecranul afișat către utilizator la bootare, dacă utilizatorul nu alege o opțiune, opțiunea implicită va fi aleasă
  \item \texttt{GRUB\_TIMEOUT}: după cât timp de la afișarea ecranului GRUB, timp în care utilizatorul nu a atins nici o tastă, se bootează în opțiunea implicită;
    valoarea \texttt{-1} înseamnă așteptare infinită până la alegerea explicită a unei opțiuni
  \item \texttt{GRUB\_TIMEOUT\_STYLE}: precizează cum va fi afișat ecranul de boot până la expirarea timpului de afișare;
    valoarea \texttt{hidden} înseamnă că nu se afișează nici un ecran, și după 10 secunde se bootează opțiunea implicită;
    în acel interval de 10 secunde utilizatorul poate apăsa tasta \texttt{Esc} pentru a cauza afișarea ecranului de boot
  \item \texttt{GRUB\_TIMEOUT\_LINUX\_DEFAULT}: adaugă opțiunile respective nucleului când bootează
\end{itemize}

În directorul \file{/etc/grub.d/} sunt fișiere de configurare pentru opțiuni GRUB.
 Multe fișiere sunt predefinite pentru configurări dedicate.
Dacă dorim să facem configurări personale vom completa fișierele \file{/etc/grub.d/40\_custom} sau \file{/etc/grub.d/41\_custom}.
Aceste fișiere sunt citite literal și adăugate la sfârșitul fișierului \file{/boot/grub/grub.cfg}.
...
Rolul bootloaderului este de a permite utilizatorului să selecteze un sistem de operare în care să booteze, să configureze opțiunile acelui sistem de operare și apoi să booteze în sistemul de operare.

Bootloaderul va încărca un fișier numit \textbf{imagine de kernel}, care conține codul și datele sistemului de operare, și apoi va transfera controlul către acesta.
Imaginea de kernel (sau imaginea de nucleu de sisteme de operare) este practic un executabil care conține implementarea nucleului (kernelului), componenta critică în funcționarea sistemului.

Nucleul realizează configurarea componentelor hardware ale sistemului, încarcă driverele de dispozitiv (\textit{device drivere}), montează sistemul de fișiere rădăcină și apoi pornește procesul \texttt{init}, primul proces al sistemului, care, la rândul său, va porni alte procese.
...
Într-o configurație tipică, fișierul reprezentând imaginea de kernel în Linux se găsește în \file{/boot/vmlinuz-$<$versiune$>$}, unde \texttt{$<$versiune$>$} este versiunea de sistem de operare.
Un exemplu este \file{/boot/vmlinuz-4.15.0-29-generic}.
...
Driverele sunt localizate, în Linux, în directorul \file{/lib/modules/$<$version$>$/kernel} unde \texttt{$<$version$>$} este versiunea de sistem de operare.
Driverele corespunzătoare imaginii \file{/boot/vmlinuz-4.15.0-29-generic} se găsesc în directorul \file{/lib/modules/4.15.0-29-generic/kernel/}.
...
\begin{itemize}
  ...
  \item încărcarea unui modul de kernel nou, folosind utilitarul \cmd{insmod} sau utilitarul \cmd{modprobe}
  \item descărcarea unui modul de kernel existent, folosind utilitarul \cmd{rmmod} sau comanda \cmd{modprobe -r}
...
Comanda \cmd{insmod} primește ca argumente calea completă către fișierul ce conține modulul de kernel în vreme ce comanda modprobe primește numele modulului și apoi îl caută în directorul \file{/lib/modules/$<$version$>$}.

Dacă dorim încărcate anumite versiuni de module de kernel la pornirea sistemului, sau dacă dorim să împiedicăm încărcarea anumitor module (\textit{blacklisting}) atunci vom realiza configurări în fișierul \file{/etc/modules} sau în directorul \file{/etc/modprobe.d/}.

În directorul \file{/boot/} pe lângă imagini de kernel, apar fișiere numite \file{initrd.img-$<$version$>$}.
...
Imaginea de ram disk este încărcată de bootloader, așa cum am văzut în ecranul de configurare a opțiunii GRUB din \labelindexref{Figura}{fig:boot:edit-grub}.
...
Imaginea de ram disk este generată la instalarea unei versiuni noi de kernel și poate fi generată la nevoie cu ajutorul comenzii \cmd{mkinitramfs}.
...
Comanda echivalentă folosită pentru bootarea kernelului, împreună cu opțiunile aferente, se găsește în fișierul \file{/proc/cmdline}:
...
\begin{itemize}
  \item opțiunile \texttt{debug}, \texttt{verbose}, \texttt{quiet} care configurează nivelul de detaliu al mesajelor de boot afișate la pornirea kernelului
  \item opțiunea \texttt{splash} activează splashscreenul la pornirea sistemului, în loc să afișeze mesajele de boot
  \item opțiunea \texttt{noacpi} dezactivează ACPI \abbrev{ACPI}{Advanced Configuration and Power Interface} (\textit{Advanced Configuration and Power Interface}) o componentă PC care uneori este cauzatoare de probleme
\end{itemize}
...
După ce sistemul de operare a pornit, putem afla informații despre acesta.
Pe lângă aflarea liniei de comandă echivalente din fișierul \file{/proc/cmdline}, mai putem afla versiunea nucleului care rulează folosind \cmd{uname}:
...
De asemenea, pentru depanare sau informare, nucleul transmite informații care se jurnalizează.
Pentru accesarea jurnalului folosim comanda \cmd{dmesg}, ca mai jos:
...
După ce nucleul și driverele sunt încărcate și sistemul de operare este inițializat, trebuie pornite primele procese.
Pentru început se pornește primul proces, numit generic \textbf{init}, care apoi pornește alte procese.
Așa cum am precizat în \labelindexref{Secțiunea}{sec:process:linux-hierarchy}, procesul init este în vârful ierarhiei proceselor având ca roluri crearea primelor procese din sistemul de operare și adoptarea proceselor orfane.

În mod tradițional, init era procesul pornit din executabilul \file{/sbin/init} și se numea System V init, după numele unei variante importante de UNIX (UNIX System V).
În mod curent implementarea cea mai răspândită de init este \textbf{systemd}, cu executabilul aflat în \file{/lib/systemd/systemd}:
...
În systemd, serviciile, împreună cu alte componente, sunt numite \texttt{units}.
Pentru a vizualiza toate unitățile systemd, folosim comanda \cmd{systemctl list-units}:
...
Dacă dorim să afișăm informații, oprim, porni sau reporni servicii, putem folosi fie interfața clasică \file{/etc/init.d/}, fie comanda \cmd{service}, fie comanda \cmd{systemctl}.
...
Dacă dorim adăugarea de servicii în sistem putem să le adăugăm în configurarea \cmd{systemd} sau putem folosi \cmd{supervisor}.
Vom detalia în \labelindexref{Secțiunea}{sec:auto:system:service}.
...
Pe un sistem fără interfață grafică se vede implicit primul terminal putându-se accesa celelalte terminale folosind combinațiile de taste \texttt{Ctrl+Alt+F2}, \ldots, \texttt{Ctrl+Alt+F6}.
 Terminalul virtual curent poate fi schimbat și cu ajutorul comenzii \cmd{chvt n}, unde \texttt{n} ia valori de la 1 la numărul de terminale virtuale alocate de kernel.

Mediul grafic, în cazul în care este prezent, poate porni în locul consolei text.
 Acesta va fi accesibil de obicei prin \texttt{Ctrl+Alt+F1} sau \texttt{Ctrl+Alt+F7}, dar orice configurație este posibilă.
...
\begin{enumerate}
  \item systemd pornește procese numite \cmd{getty} care reprezintă interfața de comunicare cu terminalul virtual.
  \item \cmd{getty} afișează un prompt de utilizator unde se introduce numele de utilizator și parola.
  \item \cmd{getty} pornește procesul \cmd{login} care citește parola utilizatorului și o verifică în fișierul \file{/etc/shadow}
  \item dacă parola este corectă, procesul \cmd{login} pornește shellul descris ca ultima coloana din linia corespunzătoare utilizatorului în fișierul \file{/etc/passwd}
\end{enumerate}
...
Observăm în cadrul comenzii că sunt cinci terminale virtuale, de la \texttt{tty2} până la \texttt{tty6}.
Putem observa și procesele aferente celor cinci terminale virtuale, gestionate de procesul \cmd{agetty}, o implementare de \cmd{getty}.
...
În toate situațiile shellul folosește niște fișiere de configurare specifice care personalizează experiența utilizatorului în linia de comandă, așa cum am prezentat în \labelindexref{Secțiunea}{sec:cli:shell-start:config}.
...
În \labelindexref{Secțiunea}{sec:user:linux-password-recovery} și \labelindexref{Secțiunea}{sec:user:windows-password-recovery} am precizat modurile uzuale de resetare a parolei administrative și în Linux și în Windows.
În Linux folosim opțiunea \texttt{init=/bin/bash} la bootarea sistemului de operare, editând intrarea în bootloader, sau folosim un USB stick bootabil sau CD-ROM \abbrev{CD-ROM}{Compact Disc - Read-Only Memory} (\textit{Compact Disc - Read-Only Memory}) sau DVD-ROM \abbrev{DVD-ROM}{Digital Video Disc - Read-Only Memory} (\textit{Digital Video Disc - Read-Only Memory}).
...
O formă mai directă de a trece de parola de firmware de boot sau cea de bootloader este detașarea discului din unitate și atașarea acestuia la o altă unitate.
...
\begin{enumerate}
  \item Ștergerea vechilor componentele de pe stickul USB.
    Presupunând că stickul USB este accesibil prin intrarea \file{/dev/sdb}, rulăm comanda

\begin{screen}
wipefs -a /dev/sdb
\end{screen}

    \cmd{wipefs} va detecta și va șterge toate semnăturile de sisteme de fișiere și tabelele de partiții prezente pe \file{/dev/sdb}.

  \item Crearea tabelei de partiții și apoi unei partiții FAT32 și marcarea acesteia ca fiind bootabilă.
    Pentru aceasta folosim utilitarul \cmd{gparted} (cu interfața grafică), accesând opțiunile:
    \begin{enumerate}
...
Pentru scrierea de informații pe stickul USB, descărcăm un fișier \texttt{.iso} cu distribuția Linux dorită.
Apoi folosim utilitarul \cmd{unetbootin} (pe orice platformă)\footnote{\url{https://unetbootin.github.io}}, cu interfață grafică.
...
În interfața \cmd{unetbootin} selectăm fișierul \texttt{.iso} pe care dorim să îl scriem și destinația unde este montat stickul, adică \file{/mnt/sdb1} în cazul nostru.
...
\end{screen}
  ...
  \item Se copiază tot conținutul din imaginea de Windows pe stickul USB:
...
\end{screen}
  ...
  \item Se generează fișierul \file{/mnt/sdb1/boot/grub/grub.cfg} cu următorul conținut:
...
\end{screen}
    ...
    Din conținutul fișierului \texttt{.iso} ne interesează fișierele \texttt{initrd} și \texttt{linux}, adică imaginea de ram disk și imaginea de kernel.
...
\end{screen}
    ...
    În configurarea GRUB am folosit UUID-ul partiției \file{/dev/sda1}, partiția unde se găsește directorul \file{/boot} (\texttt{a14d0991-a3d8-48d6-ac8c-327d1a524501}) pe care l-am descoperit cu ajutorul comenzii blkid.
Am folosit construcția \texttt{(hd0,msdos1)} pentru a identifica prima partiție din primul hard disk, adică \file{/dev/sda1}.
...
Windows Boot Manager este o componentă generică ce permite selectarea între mai multe instanțe de sistem de operare Windows de pe un sistem dat.
Windows Boot Manager prezintă utilizatorului opțiunile de sistem de operare pe care să booteze;
după ce utilizatorul alege opțiunea respectivă, Windows Boot Manager încarcă bootloaderul aferent și acesta, ulterior, va încărca sistemul de operare.

Opțiunile de boot, însemnând ce sisteme de operare sunt disponibile și ce opțiuni de boot sunt folosite pentru un sistem de operare, sunt reținute într-o zonă numită BCD \abbrev{BCD}{Boot Configuration Data} (\textit{Boot Configuration Data}), zonă ce poate fi editată folosind \cmd{BCDEdit.exe}, un utilitar inclus în Windows.

Detalii despre procesul de boot în Windows găsiți în documentație\footnote{\url{https://docs.microsoft.com/en-us/windows-hardware/drivers/devtest/introduction-to-boot-options}}.
